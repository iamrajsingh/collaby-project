...
class ContactListing extends Component {
  ...
  fetchContactLists = async () => {
    try {
      const response = await apiService.get("admin/contact");

      if (response.data && Array.isArray(response.data)) {
        // Filter out items with empty data
        const filteredLists = response.data.filter((list) => list.name);
  ...
  handleDeleteConfirmed = async () => {
    ...
    try {
      const response = await apiService.post(
        "admin/contact/delete?id=" + selectedListId + ""
      );
  ...
  handleDownload = () => {
    ...
    const worksheet = XLSX.utils.json_to_sheet(lists.map(item => ({
      Name: item.name,
        ...
        <div className="dashBoardSec">
          ...
          <div className="middleSec catbyMdl">
            <div className="headingD">
              ...
              <button className="btn btn-default pull-right" onClick={this.handleDownload}>
                <i className="fa fa-download"></i> Export Contact
              </button>
            </div>
                          ...
                          {lists &&
                            lists.map((list) => (
                              <tr className="trBack" key={list.id}>
                                ...
                                <td>{list.mobile}</td>
                                <td>{list.email_id}</td>
                                {list.contact_medium ? <td>{list.contact_medium}</td> : <td>Not Available</td>}
                                {list.contact_link ? <td>{list.contact_link}</td> : <td>Not Available</td>}
                                <td>{list.query}</td>
                              </tr>
                            ))}
                        </tbody>
