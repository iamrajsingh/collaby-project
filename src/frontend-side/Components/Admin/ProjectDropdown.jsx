...
class ProjectDropdown extends Component {
  ...
  handleProjectChange = (selectedProjectKeyword) => {
    this.setState({ selectedProjectKeyword });
    this.props.onChangeProjectKeywordProp(selectedProjectKeyword);
  };

  componentDidUpdate(prevProps, prevState) {
    // Check if the data prop has changed
    if (
      this.props.selectedProjectKeywordProp !==
      prevProps.selectedProjectKeywordProp
    ) {
      // Perform actions or side effects here

      const myArray = this.props?.selectedProjectKeywordProp?.split(","); //["a","b"]
      // const myArrOfObjects = myArray.map((el) => {
      //   return {
      //     label: el,
      //     value: el,
      //   };
    ...
    return (
      <div className="col-md-6 col-sm-6 col-xs-12">
        <div className="form-group">
          <label htmlFor="project_keyword">Project Keyword:</label>
          <Select
            name="project_keyword"
            options={this.state.keywords}
            value={this.state.selectedProjectKeyword}
